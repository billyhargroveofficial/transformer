...
![Архитектура mini-GPT](architecture.png)

Схема целиком сгенерирована из кода (`arch_diagram.py`, matplotlib): макро-проход,
разобранный декодер-блок с residual-потоком, внутренности self-attention и бюджет
параметров.
...
| `arch_diagram.py` | рисует `architecture.png` — подробную схему архитектуры |

## Запуск

```bash
python -m venv .venv && .venv/bin/pip install mlx numpy matplotlib
...
# перерисовать схему архитектуры
